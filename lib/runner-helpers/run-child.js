...
const sumanOpts = global.sumanOpts = (global.sumanOpts || JSON.parse(process.env.SUMAN_OPTS));
const usingRunner = global.usingRunner = true;
const projectRoot = global.projectRoot = process.env.SUMAN_PROJECT_ROOT;

debug(' => child args => ', util.inspect(process.argv));
debug(' => child env => ', util.inspect(process.env));
