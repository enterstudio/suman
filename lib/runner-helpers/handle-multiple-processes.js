...
module.exports = function (runnerObj, tableRows, messages, forkedCPs, handleMessage, beforeExitRunOncePost, makeExit) {
  ...
  return function runSingleOrMultipleDirs(runObj) {
    ...
    const args = global._suman.userArgs;
    ...
    const isStdoutSilent = global.sumanOpts.stdout_silent || global.sumanOpts.silent;
    const isStderrSilent = global.sumanOpts.silent;
    ...
    fileObjArray.forEach(function (fileShortAndFull, index) {
      ...
      function run() {
        ...
        if (runnerObj.bailed) {
          ...
          if (global.sumanOpts.verbosity > 6) {
        ...
        if (global.sumanOpts.coverage && '.js' === extname) {
          let coverageDir = path.resolve(global.projectRoot + '/coverage/' + String(shortFile).replace(/\//g, '-'));
        ...
        if (n.stdio) {
          ...
          if (global.sumanOpts.inherit_stdio) {
        ...
        n.once('exit', function (code, signal) {
          ...
          if ((runnerObj.bailed = (code > 0 && global.sumanOpts.bail)) ||
            ...
            if (runnerObj.bailed) {
              ...
              if(global.sumanOpts.verbosity > 4){
                console.log('\n\n');
            ...
            function waitForTAP() {
              async.parallel([
                ...
                function (cb) {
                  if (global.sumanOpts.coverage && !global.sumanOpts.no_report) {
                    console.log('\n');
                    ...
                    let coverageDir = path.resolve(global.projectRoot + '/coverage');
                    const k = cp.spawn(global._suman.istanbulExecPath,
            ...
            if (su.isSumanDebug()) {
              console.log(' => Time required to release next test(s) => ',
                Date.now() - global.timeOfMostRecentExit, 'ms');
            }
          }
        });
