...
module.exports = opts => {
  ...
  if (err) {
    console.log('\n','=> Suman message => Suman will attempt to install itself to the project in your current working directory.');
  }
  else {
  ...
  if (sumanAlreadyInittedBecauseConfFileExists && !force) {
    ...
    console.log(colors.red(' => Before you use --force/--fforce options, it\'s always a good idea to run a commit/tag with your version control system.') + '\n');
    console.log(colors.red.bold(' => Should you choose to reinitialize, Suman will write out folders with a timestamp for uniqueness,\n    and will not delete' +
  ...
  async.series({
    ...
    console.log([ '=> Notice the new directory called "suman" in the root of your project.',
      ...
      ' => http://sumanjs.github.io/tutorial-01-getting-started.html' ].map((l, index, a) => {
