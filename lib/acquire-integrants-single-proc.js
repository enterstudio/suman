...
module.exports = function acquireDepsOriginal(deps, depConfiguration, cb) {
  ...
  deps.forEach(dep => {
    ...
    if (includes(constants.SUMAN_HARD_LIST, dep && String(dep)) && String(dep) in depConfiguration) {
    ...
    else if (includes(constants.CORE_MODULE_LIST, dep && String(dep)) && String(dep) in depConfiguration) {
    ...
    // }
      ...
      if (!obj[dep]) {
        ...
        // let deps = Object.keys(depConfiguration || {}).map(function (item) {
        //   return ' "' + item + '" ';
        // });
  ...
  Promise.all(providers.map(provider => provider.call(null))).then(
    function (deps) {
