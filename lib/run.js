  ...
  async.parallel({
    ...
    removeCoverageDir: function (cb) {
      ...
      if (global.sumanOpts.coverage) {
        const covDir = path.resolve(global.projectRoot + '/coverage');
    ...
    getIstanbulPath: function (cb) {
      if (global.sumanOpts.coverage) {
        getIstanbulPath(function (err, p) {
          err ? cb(err) :
            global._suman.istanbulExecPath = p;
          cb();
        });
      }
      else {
    ...
    getFilesToRun: function (cb) {
      findFilesToRun(paths, opts, function (err, obj) {
        ...
        su.findSumanMarkers(['@run.sh', '@transform.sh'], process.env.TEST_SRC_DIR, obj.files,
          function (err, map) {
            global._suman.markersMap = map;
            fs.writeFile(global.sumanHelperDirRoot + '/suman-map.json', JSON.stringify(map), function(){
              cb(err, obj);
            });
          });
      });
